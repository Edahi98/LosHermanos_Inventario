...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Editar Artículo</h1>
            ...
            <form action="{{ route('articulos.update', $articulo->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="nombre_articulo" id="nombre_articulo" value="{{ old('nombre_articulo', $articulo->nombre_articulo) }}" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Ingrese el nombre del artículo">
                    </div>
                    <div>
                        <label for="modelos_compatibles" class="block text-gray-700 text-sm font-bold mb-2">Modelos Compatibles:</label>
                        <textarea name="modelos_compatibles" id="modelos_compatibles" rows="3" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Ingrese los modelos compatibles">{{ old('modelos_compatibles', $articulo->modelos_compatibles) }}</textarea>
                    </div>
                    <div>
                        <label for="precio_venta" class="block text-gray-700 text-sm font-bold mb-2">Precio de Venta:</label>
                        <input type="number" step="0.01" name="precio_venta" id="precio_venta" value="{{ old('precio_venta', $articulo->precio_venta) }}" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Ingrese el precio de venta">
                    </div>
                    <div>
                        <label for="precio_compra" class="block text-gray-700 text-sm font-bold mb-2">Precio de Compra:</label>
                        <input type="number" step="0.01" name="precio_compra" id="precio_compra" value="{{ old('precio_compra', $articulo->precio_compra) }}" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Ingrese el precio de compra">
                    </div>
                    <div>
                        <label for="stok" class="block text-gray-700 text-sm font-bold mb-2">Stock:</label>
                        <input type="number" name="stok" id="stok" value="{{ old('stok', $articulo->stok) }}" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="Ingrese la cantidad en stock">
                    </div>
                    <div>
                        <label for="fecha_publicacion" class="block text-gray-700 text-sm font-bold mb-2">Fecha de Publicación:</label>
                        <input type="date" name="fecha_publicacion" id="fecha_publicacion" value="{{ old('fecha_publicacion', $articulo->fecha_publicacion) }}" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    </div>
                    <div>
                        ...
                        <select name="categoria_id" id="categoria_id" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            @foreach ($categorias as $categoria)
                                <option value="{{ $categoria->id }}" @if($articulo->categoria_id == $categoria->id) selected @endif>{{ $categoria->nombre_categoria }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="marca_id" id="marca_id" class="shadow-sm appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            @foreach ($marcas as $marca)
                                <option value="{{ $marca->id }}" @if($articulo->marca_id == $marca->id) selected @endif>{{ $marca->nombre_marca }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
